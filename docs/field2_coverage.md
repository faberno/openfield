...
| `xdc_concave.m` | Concave/focused circular aperture | `ConcavePiston` | Implemented | Field II geometry validation passes; `calc_h` edge-sample parity remains open. |
| `xdc_linear_array.m` | Flat linear array | `LinearArray` | Implemented | Geometry/tessellation implemented. |
| `xdc_focused_array.m` | Linear array with mechanical elevation focus | `FocusedLinearArray` | Implemented | Geometry and `calc_h` validation pass. |
| `xdc_linear_multirow.m` | Flat multirow linear array | `LinearMultirowArray` | Implemented | Geometry/tessellation implemented. |
| `xdc_focused_multirow.m` | Multirow linear array with mechanical elevation focus | `FocusedMultirowArray` | Implemented | Geometry and `calc_h` validation pass. |
| `xdc_convex_array.m` | Convex linear array | `ConvexArray` | Implemented | Geometry and `calc_h` validation pass. |
| `xdc_convex_focused_array.m` | Convex array with mechanical elevation focus | `ConvexFocusedArray` | Implemented | Field II geometry validation passes; `calc_h` edge-sample parity remains open. |
| `xdc_convex_focused_multirow.m` | Convex multirow array with mechanical elevation focus | `ConvexFocusedMultirowArray` | Implemented | Field II geometry validation passes; `calc_h` edge-sample parity remains open. |
...
| `xdc_rectangles.m` | Aperture from rectangular patches | `RectangleAperture`, `RectangleAperture.from_fieldii_rectangles()` | Implemented | Field II geometry validation and `calc_h` validation pass for custom rectangle apertures. |
| `xdc_triangles.m` | Aperture from triangular patches | `TriangleAperture`, `TriangleAperture.from_fieldii_triangles()` | Partial | Parser/exporter and event-split flat-polygon response exist; strict Field II parity is still open because Field II's default fast triangle integration has low-sampling edge differences. |
| `xdc_lines.m` | Flat aperture from line-bounded half planes | `LineBoundedAperture`, `LineBoundedAperture.from_fieldii_lines()` | Partial | Half-plane clipping, Field II-format import/export, and event-split flat-polygon response exist; strict Field II parity is still open because Field II's default fast line integration has low-sampling edge differences. |

## Aperture Configuration And Manipulation

| Field II file | Field II functionality | `openfield` equivalent | Status | Notes |
|---|---|---|---|---|
| `xdc_focus.m` | Fixed focus timeline from focal points | `FixedFocus`, `FocusTimeline`, `Aperture.focused_at()`, `Aperture.with_focus_timeline()` | Partial | Fixed focus and time-indexed focal points exist; strict small-case validation passes, but CPU reference uses the active row at time zero. |
...
| `xdc_focus_times.m` | User-supplied focus delay timeline | `DelayTimeline`, `Aperture.with_delay_timeline()` | Partial | Container and CPU time-zero use exist; strict small-case Field II validation passes. |
| `xdc_times_focus.m` | Alias/duplicate for user-supplied focus delay timeline | `DelayTimeline`, `Aperture.with_delay_timeline()` | Partial | Field II file duplicates `xdc_focus_times` behavior; strict small-case Field II validation passes through `DelayTimeline`. |
| `xdc_apodization.m` | Physical-element apodization timeline | `Apodization`, `Aperture.with_apodization()` | Implemented | CPU `calc_h` applies the active row at time zero; strict small-case Field II validation passes. |
| `xdc_impulse.m` | Aperture impulse response | `Aperture.with_impulse_response()`, `Waveform`, `ToneBurst` | Implemented | Used by emitted-pressure and scatterer calculations. |
| `xdc_excitation.m` | Aperture excitation pulse | `Aperture.with_excitation()`, `Waveform`, `ToneBurst` | Implemented | Used by emitted-pressure and scatterer calculations. |
| `xdc_baffle.m` | Rigid or soft baffle condition | `Baffle`, `Aperture.with_baffle()` | Implemented | Rigid default and soft obliquity path pass strict small-case Field II validation. |
...
| `ele_apodization.m` | Per-subelement apodization within physical elements | `SubElementApodization`, `Aperture.with_subelement_apodization()` | Implemented | CPU `calc_h` applies one weight per subelement in aperture order; strict small-case Field II validation passes. |
| `ele_delay.m` | Per-subelement delay within physical elements | `SubElementDelays`, `Aperture.with_subelement_delays()` | Implemented | CPU `calc_h` applies one extra delay per subelement; strict small-case Field II validation passes. |
| `ele_waveform.m` | Per-physical-element waveform | `ElementWaveforms`, `Aperture.with_element_waveforms()` | Implemented | CPU emitted-pressure path supports per-element transmit waveforms; strict small-case Field II validation passes. |

## Field And Signal Calculations

| Field II file | Field II functionality | `openfield` equivalent | Status | Notes |
|---|---|---|---|---|
| `calc_h.m` | Spatial impulse response | `Simulation.spatial_impulse_response()` / `physics.spatial_impulse_response()` | Partial | CPU projected-rectangle path passes strict Field II validation for flat rectangular cases and plain convex arrays; convex-focused, concave, triangle, and line-bounded kernels remain expected failures. |
| `calc_hp.m` | Emitted pressure field | `Simulation.emitted_pressure()` / `physics.emitted_pressure()` | Implemented | CPU reference path passes strict small-case Field II validation. |
...
- Strict Field II geometry validation for linear, sparse 2D rectangular, convex, convex-focused, and concave arrays.
- Strict Field II geometry and `calc_h` validation for a custom rectangle aperture.
...
- CPU reference spatial impulse response path with Field II projected-rectangle parity for covered flat rectangular cases.
...
- Baffle, per-subelement apodization/delay, and per-element waveform containers with CPU reference hooks and strict small-case validation.
...
- Convex-focused and concave curved-aperture `calc_h` edge-sample parity with
  Field II's undocumented approximate rectangle kernel.
- Triangle-primitive and line-bounded-primitive `calc_h` parity with Field II's
  default fast integration mode. The openfield exact flat-polygon path is now
  event-split and shape-consistent across triangle/line representations.
